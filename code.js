...
function receiveMessage(data) {
  // コマンド集
    var commands = [
      [actionSignOut, /(bye|night|バ[ー〜ァ]*イ|ば[ー〜ぁ]*い|おやすみ|お[つっ]ー|おつ|さらば|お先|お疲|帰|乙|bye|night|(c|see)\s*(u|you)|退勤|ごきげんよ|グ[ッ]?バイ|tạm biệt)/i],
//      ['actionWhoIsOff', /(だれ|誰|who\s*is).*(休|やす(ま|み|む))/i],
//      ['actionWhoIsIn', /(だれ|誰|who\s*is)/i],
      [actionCancelOff, /((休|やす(ま|み|む)|休暇).*(キャンセル|消|止|やめ|ません)|cancel off|cancel)/i],
      [actionOff, /(休|やす(ま|み|む)|休暇|off)/i],
      [actionSignIn, /(chào|おはよう|モ[ー〜]+ニン|も[ー〜]+にん|おっは|おは|へろ|はろ|ヘロ|ハロ|hi|hello|morning|出勤)/i],
      [actionConfirmSignIn, /__confirmSignIn__/i],
      [actionConfirmSignOut, /__confirmSignOut__/i],
    ];
      ...
      case 'canceloff':
        var row = sheet.getRange(r,c,1,3).getValues()[0]
          if (row[0] == '-' && row[2] == 'off'){
            ...
            sheet.getRange(r,c,1,row.length).setValues([row]);
            Skype.send('<at id="'+skypeId+'">@%s</at> *' +dateStr+ '*の休暇を取り消しました' + dateStr, data);
          }else{
...
// 出勤していない人にメッセージを送る
function actionConfirmSignOut(data) {
  var holidays = Settings.get("休日");
  var today = new Date(); //new Date(2020, 7-1, 29);
  var wday = parseInt(Utilities.formatDate(today, "GMT+0700", "u"));
  var wdayStr = '月火水木金土日'.charAt(wday-1);
